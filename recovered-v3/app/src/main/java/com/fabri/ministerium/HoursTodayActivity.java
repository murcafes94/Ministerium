...
public class HoursTodayActivity extends ThemedActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        findViewById(R.id.btnCommonOffices).setOnClickListener(v ->
                openNamedSantoral("OFICIOS COMUNES"));
        findViewById(R.id.btnOfficeDead).setOnClickListener(v ->
                openNamedSantoral("OFICIO DE DIFUNTOS"));
        findViewById(R.id.btnVolumes).setOnClickListener(v ->
    ...
    private void loadDate() {
        ...
        executor.submit(() -> {
            try {
                LiturgicalDay day = LiturgicalResolver.resolve(
                        getApplicationContext(), request);
                List<HourEntry> entries = DailyHoursRepository.hoursFor(
                        getApplicationContext(), day.temporalOffice, request);
                runOnUiThread(() -> {
                ...
                runOnUiThread(() -> {
                    ...
                    Toast.makeText(this,
                            "No se pudo localizar la Liturgia de las Horas para esta fecha.",
    ...
    private void showDay(LiturgicalDay day, List<HourEntry> entries) {
        ...
        ((TextView) findViewById(R.id.txtDailySource)).setText(day.sourceNote
        ...
        chooser.setText(day.saintOffices.isEmpty() ? "Oficio del día"
                ...
                ? selectedProper.subtitle + " · cambiar formulario"
                : "Elegir feria o memoria libre");
    ...
    private void updateOfficeHeader() {
        ...
        String celebration = selectedProper == null
                ? currentDay.celebration : selectedProper.title;
        ...
        if (!currentDay.psalterWeek.isEmpty()) {
            details.add("Semana " + currentDay.psalterWeek + " del salterio");
        }
        ...
        findViewById(R.id.liturgicalColorDot).setBackgroundTintList(
                ColorStateList.valueOf(colorValue(color)));
        ...
        if (selectedProper == null) {
            ...
            ((TextView) findViewById(R.id.txtProperTitle)).setText(
                    "Propio de " + selectedProper.title);
            ((TextView) findViewById(R.id.txtProperSubtitle)).setText(
    ...
    private void chooseOffice() {
        ...
        if (selectedProper != null && selectedProper.requiresProperOffice()) {
            loadCommonChoices(true);
            return;
        }
        List<String> labels = new ArrayList<>();
        labels.add("Oficio de la feria · " + currentDay.celebration);
        for (HoursLink saint : currentDay.saintOffices) {
            labels.add(saint.subtitle + " · " + saint.title);
        }
        int checked = selectedProper == null
                ? 0 : currentDay.saintOffices.indexOf(selectedProper) + 1;
        new AlertDialog.Builder(this)
                .setTitle("Elegir oficio")
                .setSingleChoiceItems(labels.toArray(new String[0]), checked,
                        (dialog, which) -> {
                            selectedProper = which == 0
                                    ? null : currentDay.saintOffices.get(which - 1);
                            dialog.dismiss();
                            if (selectedProper == null) {
                                selectedCommon = null;
                                commonChoices = Collections.emptyList();
    ...
    private void loadCommonChoices(boolean showChooser) {
        ...
        new AlertDialog.Builder(this)
                .setTitle("Elegir formulario de la celebración")
                ...
                .setOnCancelListener(dialog -> updateOfficeHeader())
                .show();
    }
    ...
    private void openHour(HourEntry entry) {
        ...
        intent.putExtra(HoursReaderActivity.EXTRA_EASTER_SEASON,
                currentDay != null && currentDay.temporalOffice != null
                        && "easter".equals(currentDay.temporalOffice.volume.id));
        ...
        if (ordinaryWeek > 0) {
            ...
            intent.putExtra(HoursReaderActivity.EXTRA_LECTIONARY_CYCLE,
                    LiturgicalResolver.lectionaryCycle(referenceDate));
            intent.putExtra(HoursReaderActivity.EXTRA_READINGS_YEAR,
                    referenceDate.get(Calendar.YEAR) % 2 == 0 ? 2 : 1);
        }
        if (selectedProper != null && !sundayFirstVespers
                && ("invitatory".equals(entry.key) || "office".equals(entry.key)
                    || "lauds".equals(entry.key)
                    || "vespers".equals(entry.key)
                    || selectedProper.isFeastOrSolemnity()
                    && ("terce".equals(entry.key) || "sext".equals(entry.key)
                    || "none".equals(entry.key)))) {
            intent.putExtra(HoursReaderActivity.EXTRA_MEMORY_SAINT_VOLUME_ID,
                    selectedProper.volume.id);
            intent.putExtra(HoursReaderActivity.EXTRA_MEMORY_SAINT_TOC_INDEX,
                    selectedProper.tocIndex);
            intent.putExtra(HoursReaderActivity.EXTRA_MEMORY_SAINT_TITLE,
            ...
            if (selectedCommon != null) {
                intent.putExtra(HoursReaderActivity.EXTRA_MEMORY_COMMON_FILE,
                        selectedCommon.filePath);
                intent.putExtra(HoursReaderActivity.EXTRA_MEMORY_COMMON_FRAGMENT,
                        selectedCommon.fragment);
                intent.putExtra(HoursReaderActivity.EXTRA_MEMORY_COMMON_TITLE,
    ...
    private void openCombined() {
        ...
        if (selectedProper != null) {
            ...
            intent.putExtra(CombinedHoursActivity.EXTRA_SAINT_RANK,
                    selectedProper.liturgicalRank);
        }
        if (selectedCommon != null) {
            intent.putExtra(CombinedHoursActivity.EXTRA_COMMON_FILE, selectedCommon.filePath);
            intent.putExtra(CombinedHoursActivity.EXTRA_COMMON_FRAGMENT,
                    selectedCommon.fragment);
    ...
    private void openNamedSantoral(String title) {
        try {
            HoursVolume volume = HoursRepository.find("sanctoral");
            int index = EpubUtils.findEntryIndex(this, volume, title);
            if (index < 0) throw new IllegalStateException();
            openLink(new HoursLink(volume, index, title, "Santoral"));
        } catch (Exception error) {
            Toast.makeText(this, "No se pudo abrir este oficio.", Toast.LENGTH_SHORT).show();
        }
    }
    ...
    private static int colorValue(String color) {
        if ("Blanco".equalsIgnoreCase(color)) return Color.parseColor("#E8E0CF");
        if ("Rojo".equalsIgnoreCase(color)) return Color.parseColor("#B3262E");
        if ("Morado".equalsIgnoreCase(color)) return Color.parseColor("#70408C");

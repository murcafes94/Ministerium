...
public final class MissalCompactView {
    ...
    public static void inject(WebView webView) {
        ...
        String script = "(function(){if(document.getElementById('ministerium-compact-missal-style'))return;"
                ...
                + "function rubric(t){return /^(el sacerdote|el diacono|el celebrante|luego|despues|a continuacion|mientras|si se |si no |si parece oportuno|cuando |entonces |en este momento|todos se |los fieles |el pueblo |terminada |acabado |seguidamente |aqui |puede |pueden )/.test(t);}"
                + "function essential(t){return /(de pie|se arrod|arrodill|inclina|genuflex|eleva|elevand|caliz|hostia|patena|imposicion|signo de la paz|fraccion|comunion|lavabo|silencio|incienso|altar|epicles|consagr|procesion|ambon|evangeliario|agua y vino|extiende las manos)/.test(t);}"
                ...
                + "if(essential(t)||t.length>320)p.classList.add('ministerium-essential-rubric');"

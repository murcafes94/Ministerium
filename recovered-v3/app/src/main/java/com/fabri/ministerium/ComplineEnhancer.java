    ...
    public static void inject(WebView webView, boolean ordained, boolean easterSeason) {
        try {
            ...
            String script = "(function(p,h,m,conclusion,penitentialConclusion){if(document.getElementById('ministerium-compline-start'))return;"
                    ...
                    + "for(var i=0;i<items.length;i++){(function(index){var b=document.createElement('button');b.type='button';b.textContent=items[index].title||('Fórmula '+(index+1));b.setAttribute('aria-pressed','false');b.onclick=function(){select(index);};nav.appendChild(b);})(i);}"
                    ...
                    + "var pi=[];for(var x=0;x<p.length;x++)pi.push({title:'Fórmula '+(x+1),text:p[x]});var penanceBox=buttons(pi,penanceTitle,'ministerium-penance');"

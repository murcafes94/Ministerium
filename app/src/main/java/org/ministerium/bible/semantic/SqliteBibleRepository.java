...
public final class SqliteBibleRepository implements BibleRepository {
    ...
    @Override
    public BibleEdition getEdition() {
        if (edition == null) {
    ...
    private String meta(String key) {
        Cursor cursor = database.rawQuery("SELECT value FROM metadata WHERE key = ? LIMIT 1",
                new String[]{key});
        try {
    ...
    @Override
    public List<BibleBook> listBooks() {
    ...
    @Override
    public List<BibleVerse> getChapter(String bookId, int chapter) {
    ...
    @Override
    public List<BibleVerse> getVerseRange(BibleReference reference) {
        if (reference == null) return Collections.emptyList();
        if (reference.getEditionId() != null
                && !reference.getEditionId().equals(getEdition().getEditionId())) {
    ...
    public List<BibleVerse> search(String query, int limit) {
        ...
        ArrayList<BibleVerse> verses = new ArrayList<>();
        Cursor cursor = database.rawQuery(
                "SELECT c.book_key, c.chapter_number, v.verse_label, v.verse_order, v.text, "
                        + "v.is_heading, v.paragraph_start "
                        + "FROM verses v JOIN chapters c ON c.id = v.chapter_id "
        ...
        try {
            while (cursor.moveToNext()) {
                verses.add(new BibleVerse(
                        getEdition().getEditionId(), cursor.getString(0), cursor.getInt(1),
    ...
    @Override
    public List<BibleToken> getTokens(String verseId) {
        ...
        Cursor cursor = database.rawQuery(
                "SELECT token_id, verse_id, position, surface, language, lemma, strong_id, morphology, source_dataset "
                        + "FROM tokens WHERE verse_id = ? ORDER BY position",
                new String[]{verseId});
        try {
            while (cursor.moveToNext()) {
                tokens.add(new BibleToken(
                        cursor.getString(0), cursor.getString(1), cursor.getInt(2), cursor.getString(3),
